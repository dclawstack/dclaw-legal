...
### How do I update DClaw Legal?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp legal --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Legal without Kubernetes?

Yes, for local development:

```bash
cd dclaw-legal/frontend && npm run dev
cd dclaw-legal/backend && uvicorn dclaw_legal.main:app --reload
```
