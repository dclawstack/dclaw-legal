...
# Check events
kubectl get events -n dclaw-legal --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-legal deployment/dclaw-legal-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-legal-db-credentials -n dclaw-legal
   ```
